import { dia, shapes, util, V, linkTools } from '@joint/core';
import './styles.css';

export const Gate = dia.Element.define('logic.Gate', {
...
export const Wire = shapes.standard.Link.define('logic.Wire', {
    attrs: {
        line: {
            strokeWidth: 2,
            targetMarker: null
...
Gate.prototype.onSignal = function (signal, handler) {
    handler(signal);
};
// The repeater delays a signal handling by 400ms
Repeater.prototype.onSignal = function (signal, handler) {
    setTimeout(function () {
        handler(signal);
    }, 400);
};
// Output element just marks itself as alive.
Output.prototype.onSignal = function (signal) {
...
const gates = {
    repeater: new Repeater({ position: { x: 410, y: 25 } }),
    or: new Or({ position: { x: 550, y: 50 } }),
    and: new And({ position: { x: 550, y: 150 } }),
    not: new Not({ position: { x: 90, y: 140 } }),
    nand: new Nand({ position: { x: 550, y: 250 } }),
    nor: new Nor({ position: { x: 270, y: 190 } }),
    xor: new Xor({ position: { x: 550, y: 200 } }),
    xnor: new Xnor({ position: { x: 550, y: 100 } }),
    input: new Input({ position: { x: 5, y: 45 } }),
    output: new Output({ position: { x: 440, y: 290 } })
};
...
function toggleLive(model, signal) {
    ...
    model.findView(paper).el.classList.toggle('live', signal > 0);
}

function broadcastSignal(gate, signal) {
    // broadcast signal to all output ports
    setTimeout(() => {
        const outboundLinks = graph.getConnectedLinks(gate, { outbound: true });
        outboundLinks.forEach(link => {
            link.set('signal', signal);
        });
    }, 0);
}
...
function initializeSignal() {
    ...
    V(paper.layers).find('.live').forEach(function (vel) {
